...
const stringFormats = {
  ...
  date(value) {
    return value.length < 200 && isoDate.test(value);
  },
  ...
  email(value, options) {
    return value.length < 255 && isemail.validate(value, options['x-isemail'] || {});
  },
  ...
  uri(value) {
    return value.length < 2083 && isuri.isValid(value);
  }
};
...
function checkFormat(name, value, format, errors) {
  if (!stringFormats.hasOwnProperty(format)) {
